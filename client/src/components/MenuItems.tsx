...
const MenuItems = ({ items, onItemDeleted, isSeller }: MenuItemsProps) => {
    ...
    try {
      const { data } = await axios.put(
        `${import.meta.env.VITE_RESTAURANT_SERVICE_URL}/api/item/status/${itemId}`,
        {},
        {
          headers: {
    ...
    const handleDelete = async (itemId: string) => {
      ...
      try {
        await axios.delete(`${import.meta.env.VITE_RESTAURANT_SERVICE_URL}/api/item/delete/${itemId}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
        });
